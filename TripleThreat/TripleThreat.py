...
def game():
    if request.method == "GET":
        # Check for new teams request
        if request.args.get('new_teams') == '1':
            session.pop('columns', None)
            session.pop('rows', None)
            session.pop('players', None)
        if 'columns' in session and 'rows' in session and 'players' in session:
        ...
        try:
            ...
            def safe_eval(val, default):
                try:
                    ...
                    return default
            game_data = {
                'columns': safe_eval(request.form.get('columns', '[]'), []),
                'rows': safe_eval(request.form.get('rows', '[]'), []),
                'answers': safe_eval(request.form.get('answers', '[]'), [['1','2','3'],['4','5','6'],['7','8','9']]),
                'players': safe_eval(request.form.get('players', '[]'), []),
                'xturn': request.form.get('xturn'),
                'filled_boxes': safe_eval(request.form.get('filled_boxes', '[]'), []),
            ...
            # If game is over (win or draw), clear session for new game on Play Again
            if game_data['winner']:
                session.pop('columns', None)
                session.pop('rows', None)
                session.pop('players', None)
            ...
            return 'An error occurred while processing your move. Please try again.', 400
